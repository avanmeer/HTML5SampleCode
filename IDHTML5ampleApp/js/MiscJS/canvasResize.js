    ...
    Plugin.prototype = {
        init: function () {
            ...
            reader.onloadend = function (e) {
                ...
                var resolution;
                var x = false;
                if (exif.hasOwnProperty('XResolution'))
                    resolution = exif['XResolution'].numerator / exif['XResolution'].denominator;

                //console.log("Resolution: " + resolution);
                //console.log("Is not IOS? " + !$this.options.isiOS);
                if (!$this.options.isPreprocessing) {
                    img.onload = function (e) {
                        $this.options.callback(reader.result, null, null);
                    }
                }
                else if ((!$this.options.isiOS && resolution == 300 && ($this.options.cardType == "DriversLicense"
                    || $this.options.cardType == "DriversLicenseDuplex"
                    || $this.options.cardType == "Passport"))
                    || (!$this.options.isiOS && resolution == 600 && $this.options.cardType == "MedicalCard"
                    || $this.options.cardType == "Barcode")) {
                    img.onload = function (e) {
                        $this.options.callback(reader.result, null, null);
                    }
                }
